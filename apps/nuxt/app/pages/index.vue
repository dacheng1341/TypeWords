      ...
      >
        ...
        <div class="mt-2 pt-3 border-t border-[var(--hw-border)] flex flex-col items-center">
          <template v-if="!userStore.isLogin">
            <button
              @click="() => { userStore.showGlobalLoginModal = true; mobileMenuOpen = false; }"
              class="w-full flex items-center justify-center px-3 h-9 rounded-lg text-[.9rem] font-semibold text-white bg-gradient-to-r from-[#7c3aed] to-[#2563eb] border-none cursor-pointer"
            >登录 / 注册</button>
          </template>
          <template v-else>
            <div class="flex items-center justify-between w-full">
              <div class="flex items-center gap-1.5 text-[.9rem] text-[var(--hw-text-2)]">
                <span class="text-[#7c3aed] text-[.6rem]">●</span>
                <span class="truncate">{{ userStore.user?.username || userStore.user?.email || '已登录' }}</span>
              </div>
              <button
                @click="handleLogout"
                class="text-[.8rem] font-medium text-[#ef4444] bg-transparent border-none py-1"
              >退出登录</button>
            </div>
          </template>
        </div>
